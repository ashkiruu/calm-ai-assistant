...
class CrossPhaseOrderTests(unittest.TestCase):
    ...
    def test_a_cross_phase_card_leading_a_task_is_rejected(self) -> None:
        ...
        task = next(
            ...
            for task in mission["tasks"]
            if task["task_id"] == "eq_home_a3_aftershock"
        )
        # Allowed: EQ-AFT-006 (after) then EQ-DUR-004 (during). Swap them.
        ...
        self.assertTrue(
            any(
                "eq_home_a3_aftershock" in error and "first protocol_id" in error
                for error in report["errors"]
            ),
            report["errors"],
        )
